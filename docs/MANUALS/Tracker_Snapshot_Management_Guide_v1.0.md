...
3. **配置任务内容**
   ```
   任务名称：Tracker Weekly Snapshot Capture
   
   执行内容：
   1. 获取当前所有项目列表
   2. 对每个项目调用 POST /api/progress/{project_id}/snapshot
   3. 记录执行结果到日志
   ```
...
PROJECT_IDS="1 2 3"  # 项目 ID 列表

for pid in $PROJECT_IDS; do
    response=$(curl -s -X POST "${TRACKER_URL}/api/progress/${pid}/snapshot")
    echo "$(date): Project ${pid} - ${response}"
done
```
      ...
      template:
        spec:
          ...
          - name: snapshot
            ...
            command: ["python3", "scripts/create_snapshot.py"]
...
```bash
#!/bin/bash
for pid in $(sqlite3 data/projects.db "SELECT id FROM projects"); do
