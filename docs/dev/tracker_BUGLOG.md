...
**影响验收项**:
- 5.1.4: PASS → 其他状态有二次确认
- 5.4.4: PASS → 其他状态有二次确认

**当前状态**: `updateTCStatus()` 函数直接调用 API，没有确认逻辑。

**修复方案**: 在 `updateTCStatus()` 函数中添加确认逻辑。
...
**影响验收项**: 5.5.3 默认值 DV1.0

**修复方案**: 将 TC Modal 中的 DV Milestone 默认值改为 DV1.0。
...
| BUG-016 | PASS → 其他状态缺少二次确认 | v0.6.0 | 待修复 |
| BUG-017 | DV Milestone 默认值不正确 | v0.6.0 | 待修复 |
