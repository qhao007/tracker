...
| EXP-007 | CP CSV 导出触发下载 | 验证 CP CSV 格式导出功能正常触发下载 | #11 | P1 |
| EXP-008 | TC CSV 导出触发下载 | 验证 TC CSV 格式导出功能正常触发下载 | #11 | P1 |
...
**注**：UI 测试已全部通过，包括新添加的 EXP-007 (CP CSV 导出) 和 EXP-008 (TC CSV 导出)。
...
**根因**: HTTP Header 中文编码问题 + 路由覆盖

**修复方案**:
1. 添加 `urllib.parse.quote` 对文件名进行 URL 编码
2. 修复 `static_files` 路由排除 `/api/` 路径
...
| 版本 | 日期 | 变更内容 | 作者 |
|------|------|----------|------|
| 1.1 | 2026-02-18 | 添加 EXP-007/008 CSV 导出测试用例，更新测试结果 | 小栗子 🌰 |
