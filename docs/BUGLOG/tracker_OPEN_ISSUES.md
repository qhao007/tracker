...
| **优先级** | P2 |
| **状态** | ✅ 已修复 |
| **问题描述** | 计划曲线 API 中状态查询大小写不匹配：`tc.status = 'Pass'`，但数据库中是 `'PASS'` |
| **根因** | 实现计划曲线功能时，未确认数据库中实际使用的状态格式 |
| **修复内容** | `dev/app/api.py:774` - `tc.status = 'Pass'` → `tc.status = 'PASS'` |
