...
## BUG-XXX: start_server_test.sh 脚本多实例问题
...
启动 `start_server_test.sh` 脚本时，如果 8081 端口已有 gunicorn 进程在运行，脚本会直接退出而不清理旧进程。导致每次调用脚本都会新增一个 gunicorn 进程，最终造成多个 8081 测试服务同时运行。

### 影响

- 8081 端口存在多个 gunicorn 进程（最多检测到 3 个）
- 资源浪费（内存、CPU）
- 日志混乱（多个进程同时写同一日志文件）
- 可能导致端口冲突或服务不稳定
...
当有 8081 gunicorn 进程存在时，`is_running()` 返回 `true`，脚本直接 `exit 0`，后续的 `pkill` 永远不会执行。

### 修复方案

调整脚本逻辑，先清理旧进程，再检查是否需要启动：
...
- `/projects/management/tracker/dev/start_server_test.sh`

### 验证结果

| 检查项 | 结果 |
|--------|------|
| 修复后执行脚本 | 只保留 1 个 master + 2 workers |
| 多次执行脚本 | 不再新增进程 |
