...
> **⚠️ 重要铁律**：**禁止通过手工命令（如 `cp`、`ln`、手动启动服务等方式）进行版本发布。**

**原因**：
1. 手工发布绕过了发布脚本的安全检查，可能导致数据丢失
2. 手工发布无法保证发布流程的完整性和可追溯性
3. 手工发布可能破坏版本管理的一致性（如软链接指向错误）

**禁止行为**：
- ❌ 手动 `cp -r` 复制代码到发布目录
- ❌ 手动 `ln -s` 创建或修改软链接
- ❌ 手动 `gunicorn` 或 `python server.py` 启动服务
- ❌ 手动 `rm` 删除发布目录或文件
- ❌ 绕过 release.py 执行任何发布相关操作

**正确做法**：
- ✅ 所有发布操作必须通过 `release.py` 或 `release_preparation.py` 执行
- ✅ 如果脚本报错，必须 **中止发布** 并通知人类用户处理
- ✅ 等待人类用户修复问题后，重新执行发布流程
