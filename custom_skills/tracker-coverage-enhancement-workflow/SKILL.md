...
| 阶段 | 子代理 | 模型 | max_turns | 推理深度 | 必加载技能 |
|------|--------|------|-----------|---------|-----------|
| 分析 | Subagent COV_A | sonnet | 2 | 高 | 无 |
| API测试 | Subagent COV_B | sonnet | **5** | 中高 | **Bash, pytest** |
| UI测试 | Subagent COV_C | sonnet | **5** | 高 | **playwright-debug, Bash, Playwright** |
| 手工测试 | Subagent COV_D | sonnet | 2 | 中 | **playwright-debug, agent-browser-usage, Bash** |
| 验收 | Subagent COV_E | opus | 2 | 高 | **Bash, Read** |

> **技能加载要求**:
> - **API测试（阶段2）**: 必须激活 Bash 和 pytest
> - **UI测试（阶段3）**: 必须激活 `playwright-debug`，获取Trace Viewer、超时问题、元素定位等调试指南，以及 Playwright 测试框架
> - **手工测试（阶段4）**: 必须激活 `playwright-debug` 和 `agent-browser-usage`，获取浏览器自动化和调试指南
> - **验收（阶段5）**: 必须激活 Bash 和 Read
...
**必须激活的完整技能列表**:
1. **playwright-debug** - Playwright 测试调试最佳实践
2. **Bash** - 运行 Playwright 测试
3. **Playwright** - UI 测试框架
...
2. **agent-browser-usage** - agent-browser 命令行浏览器自动化（必须激活）
3. **Bash** - 辅助命令
