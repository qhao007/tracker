...
| 阶段 | 子代理 | 模型 | max_retries | 推理深度 | 必加载技能 |
|------|--------|------|-------------|---------|-----------|
| 开发 | A | sonnet | 3 | 高 | minimax-skills:fullstack-dev |
| 审查 | B | opus | 3 | 极高 | **minimax-skills:frontend-dev** |
| 优化 | A | sonnet | 3 | 高 | minimax-skills:fullstack-dev |
| API测试 | C | sonnet | 3 | 中高 | - |
| UI测试 | D | sonnet | 3 | 高 | playwright-debug |
| 手工测试 | E | sonnet | 2 | 中 | playwright-debug |
| 测试验收 | F | opus | 2 | 高 | - |

> **技能加载要求**:
> - **代码开发（阶段1/3）**: 必须激活 `minimax-skills:fullstack-dev`，获取后端API和前端集成开发指南
> - **代码审查（阶段2）**: 必须激活 `minimax-skills:frontend-dev`，获取前端设计规范、动效系统、UI组件等开发指南，确保审查时遵循 Tracker 的前端设计标准
> - **UI/手工测试（阶段5/6）**: 必须激活 `playwright-debug`，获取Trace Viewer、超时问题、元素定位等调试指南
...
1. **playwright-debug** - Playwright调试最佳实践（必须激活，获取Trace Viewer、超时问题、元素定位等调试指南）
2. **agent-browser-usage** - 命令行浏览器自动化（必须激活，获取完整使用指南）
3. **Bash** - 辅助命令
